...
for ID in uniprot_id:
    print("uniprot_id", ID)
    #依次输入搜索的id，获取相应的网页源码
    html=requests.get("https://www.uniprot.org/uniprot/"+ID, headers={"Connection":"close"}, verify=False, stream=True, timeout=None)
    res=html.text
    ...
    if len(domain_positions) <= 1:
        ...
        if linker_positions:
            ...
            sequence_to_be_crawlered=requests.get("https://www.uniprot.org/uniprot/"+ID+".fasta", headers={"Connection":"close"}, verify=False, stream=True, timeout=None)
            ...
            #print("linker_region:", linker_region)                
        else:
            pass
...
with open("Linkers_fasta.txt", "r") as f:
    for line in f:
        ...
        if len(line) > 1:
            ...
            res=requests.post(url, data=data, headers=headers, verify=False, stream=True, timeout=None)
            res_text=res.text
